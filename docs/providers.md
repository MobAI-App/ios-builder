...
If the selected provider has custom workflow names in `builder.json`, rerunning
setup preserves those names and all CI files. Merge runner/workflow updates
manually in that case; `--app-id`, `--branch`, and `--set-default` still apply.
...
Explicit request/access rejections (such as an invalid app ID or denied access)
